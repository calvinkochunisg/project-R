...
#the last closing price and the last moving average value. This value will be applied to the variable distance MA7 (in percent)
  ...
  distance_MA7 <<- round((ticker_data$MA7[nrow(ticker_data)]*100/ticker_data$close[nrow(ticker_data)])-100, digits = 4)
...
#Moving average (50 days) will be included without any condition. Same for distance between ma and last close price.
  ...
  MA_50 <<- rollmean(ticker_data$close, k = 50)
  ma50 <<- c(NA [1:49], MA_50)
  ticker_data["MA50"] <<- ma50
  
  distance_MA50 <<- round((ticker_data$MA50[nrow(ticker_data)]*100/ticker_data$close[nrow(ticker_data)])-100, digits = 4)
...
#Else, a column in the df with the name MA200 will be filled blank
  ...
  MA_200 <<- rollmean(ticker_data$close, k = 200)
  ma200 <<- c(NA [1:199],MA_200)
  ticker_data["MA200"] <<- ma200
...
#Check if MA_200 is existent, if so, formula is applied to calculate differnce between ma and last close price. If not the sentce below will be printed.
  ...
  distance_MA200 <<- round((ticker_data$MA200[nrow(ticker_data)]*100/ticker_data$close[nrow(ticker_data)])-100, digits = 4)
...
#Generate the Plot Caption with the calculated financial Data
  ...
  if(n_distinct(ticker_data["MA7"])>1){
    plot_caption <<- c(plot_caption,
                            paste("The difference between", endDate, "closing price and 7 days moving average is", distance_MA7, "percent", "\n\n", sep="")
    )
  }
  
  if(n_distinct(ticker_data["MA50"])>1){
    plot_caption <<- c(plot_caption,
                            paste("The difference between ", endDate, " closing price and 50 days moving average is ", distance_MA50, " percent", "\n\n", sep="")
    )
  }
  
  if(n_distinct(ticker_data["MA200"])>1){
    plot_caption <<- c(plot_caption,
                            paste("The difference between ", endDate, " closing price and 200 days moving average is ", distance_MA200, " percent", "\n\n", sep="")
    )
  }
